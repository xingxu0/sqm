...
main (int argc, char *argv[])
{
	...
	for (uint16_t i = 0; i < n2; i++)
		positionAlloc->Add (Vector(-600, distance2+600*1.732, 0));
	for (uint16_t i = 0; i < n3; i++)
		positionAlloc->Add (Vector(-distance2-1200, 0, 0));
	
	double x[3] = {0, -600, -1200};
	double y[3] = {0, 600*1.732, 0};
	for (uint16_t i = 0; i < bs; i++)
		positionAlloc->Add (Vector(x[i], y[i], 0));
