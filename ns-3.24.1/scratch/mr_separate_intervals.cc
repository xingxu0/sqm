...
//std::cout<<std::endl;
	...
	int new_maintain_thr = 0;
	for (uint16_t i=0; i<rate_maintain_users; ++i){
		uint64_t imsi = get_key_from_value(i, imsi_id);
		uint16_t rnti = get_key_from_value(imsi, rnti_imsi);
		
		int mcs_value = int(round(((int)(*rnti_mcs_count_selected)[rnti] == 0 ? -1 : (*rnti_mcs_selected)[rnti]*1.0/(*rnti_mcs_count_selected)[rnti])));
		if (mcs_value > 28 || mcs_value < 0) {
			std::cout<<i<<" s mcs value issue"<<mcs_value<<" "<<(int) (*rnti_mcs)[rnti]<<":"<<(*rnti_mcs_count)[rnti]<<"   ";
			mcs_value = 28;
		}
		int prb =  4*interval_requirement*maintain_thr / amc->GetTbSizeFromMcs(mcs_value, 1); // 2 is an arbitrary number
		prbs.push_back(prb);
