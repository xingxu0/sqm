...
main (int argc, char *argv[])
{
	...
	double interPacketInterval = 0.000000000001;
  ...
  //LogComponentEnable ("Packet", logLevel);
...
enum EpsBearer::Qci q;
	if (u == 0) q = EpsBearer::NGBR_VIDEO_TCP_PREMIUM;
  ...
  ApplicationContainer serverApps;
    for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      //TcpClientHelper dlClient (ueIpIface.GetAddress (u), dlPort);
        ...
        BulkSendHelper dlClient ("ns3::TcpSocketFactory",InetSocketAddress (remoteHostAddr, dlPort));
