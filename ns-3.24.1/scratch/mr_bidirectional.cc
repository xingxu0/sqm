...
//lteHelper->SetEnbDeviceAttribute ("UlBandwidth", UintegerValue (100));
	...
	ulclientApps.Get(0)->SetStartTime(Seconds(1));
	...
	for (i = clientApps.Begin ()+1; i != clientApps.End (); ++i) {
		if (n>first_started && n<n1)
		{
			...
			ulclientApps.Get(n)->SetStartTime(Seconds((n-first_started)*5.0));
			ulclientApps.Get(n)->SetStopTime(Seconds((n-first_started)*5.0 + (n1-n)*10.0));
		}
		else {
			(*i)->SetStartTime(Seconds(1));
			ulclientApps.Get(n)->SetStartTime(Seconds(1));
		}
		n += 1;
	}
	n = 1;
