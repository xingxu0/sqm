...
void dynamic_maintain() {
	...
	for (uint16_t i=0; i<n1; ++i){
		...
		std::cout<<(int)(*rnti_mcs)[rnti]<<" "<<(*rnti_rate)[rnti]<<" "<<(*rnti_prbs)[rnti]
		<<" "<<(*rnti_mcs_count_selected)[rnti]<<std::endl;
	}
	clear_prb();
	clear_mcs();
	return;
...
main (int argc, char *argv[])
{
	...
	//Config::SetDefault ("ns3::OnOffApplication::DataRate", StringValue (dataRate + "Mb/s"));
...
//lteHelper->SetEnbDeviceAttribute ("UlBandwidth", UintegerValue (100));
	...
	OnOffHelper fastclientHelper ("ns3::UdpSocketFactory", Address ());
	if (! udp)
		fastclientHelper.SetAttribute("Protocol", StringValue("ns3::TcpSocketFactory")); 
	fastclientHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
	fastclientHelper.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
	fastclientHelper.SetAttribute ("DataRate", StringValue ("3Mb/s"));	
	
	OnOffHelper slowclientHelper ("ns3::UdpSocketFactory", Address ());
	if (! udp)
		slowclientHelper.SetAttribute("Protocol", StringValue("ns3::TcpSocketFactory")); 
	slowclientHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
	slowclientHelper.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
	slowclientHelper.SetAttribute ("DataRate", StringValue (dataRate + "Mb/s"));
	...
	for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
	{
		...
		AddressValue ueAddress (InetSocketAddress (ueIpIface.GetAddress (u), dlPort));
		Ptr<Node> remoteHost = remoteHostContainer.Get (u);
		if (u % 2 == 0) {
			fastclientHelper.SetAttribute ("Remote", ueAddress);
			clientApps.Add(fastclientHelper.Install(remoteHost));
		} else {
			slowclientHelper.SetAttribute ("Remote", ueAddress);
			clientApps.Add(slowclientHelper.Install(remoteHost));
		}
