...
// assign each UE multiple TCP flows
...
main (int argc, char *argv[])
{
  ...
  inputConfig.ConfigureDefaults();
	...
	NodeContainer remoteHostContainer2;
	remoteHostContainer2.Create (numberOfNodes);
	
	//
	InternetStackHelper internet;
	internet.Install (remoteHostContainer);
	internet.Install (remoteHostContainer2);

	std::vector<PointToPointHelper*> p2pv;
	std::vector<PointToPointHelper*> p2pv2;
	// Create the internet
	...
	for (uint16_t i = 0; i < numberOfNodes; i++) {
		...
		Ptr<Node> remoteHost = remoteHostContainer2.Get (i);
		NetDeviceContainer internetDevices = p2ph->Install (pgw, remoteHost);
		Ipv4AddressHelper ipv4h;
		char buf[512];
        sprintf(buf,"1.1.%d.0", i);
		ipv4h.SetBase (buf, "255.255.255.0");
		Ipv4InterfaceContainer internetIpIfaces = ipv4h.Assign (internetDevices);
	// interface 0 is localhost, 1 is 15the p2p device
	//Ipv4Address remoteHostAddr = internetIpIfaces.GetAddress (1);
...
// bearer      
  ...
  ApplicationContainer serverApps, serverApps2;
    for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ++ulPort;
      ++otherPort;
...
for (uint32_t u = 0; u < 1; ++u)
    {
      ...
      client.SetAttribute ("MaxPackets", UintegerValue(1000000));
      */
	  Ptr<Node> remoteHost = remoteHostContainer2.Get (u);
...
/*
  ...
  for (uint8_t i=0; i<ueNodes.GetN (); ++i) {
	...
	sprintf(buff, "trace_nonsharing_%d_1.pcap", i);
	...
	sprintf(buff, "trace_nonsharing_%d_2.pcap", i);
	buffAsStdStr = buff;
	std::cout<<remoteHostContainer.Get(i)->GetNDevices()<<std::endl;
	p2pv2[i]->EnablePcap(buffAsStdStr, remoteHostContainer2.Get(i)->GetDevice(1), false, true);
  }
