...
for i in range(time):
	...
	r = sqm(bpp, admitted_sqm, new_user, current_user, last_r, 1)
	u_sqm.append(sum(r[1])*1.0)#/count_admitted_user(admitted_sqm))
	u_a_sqm.append(sum(r[1])*1.0/count_admitted_user(admitted_sqm) if count_admitted_user(admitted_sqm) else 0 )
	result_sqm.append(r)
	last_r = copy.deepcopy(r)
	
	r = sqm(bpp, admitted_sqm2, new_user, current_user, last_r2, 2)
	u_sqm2.append(sum(r[1])*1.0)#/count_admitted_user(admitted_sqm))
	u_a_sqm2.append(sum(r[1])*1.0/count_admitted_user(admitted_sqm2) if count_admitted_user(admitted_sqm2) else 0 )
	result_sqm2.append(r)
	last_r2 = copy.deepcopy(r)
	
	r = sqm(bpp, admitted_sqm3, new_user, current_user, last_r2, 3)
	u_sqm3.append(sum(r[1])*1.0)#/count_admitted_user(admitted_sqm))
	u_a_sqm3.append(sum(r[1])*1.0/count_admitted_user(admitted_sqm3) if count_admitted_user(admitted_sqm3) else 0 )
	result_sqm3.append(r)
	last_r3 = copy.deepcopy(r)
